...
class DDPM(pl.LightningModule):
    ...
    def loss(self, x):
        ...
        squared_norm_preds = torch.mean(torch.sum(eps_pred**2, dim=1)) / dim_
        cov_mat = eps_pred.T@eps_pred/eps_pred.shape[0]
        non_diag = (cov_mat[0][1] + cov_mat[1][0]) / dim_
        one = torch.tensor(1.0, requires_grad=True)
        
        norm_loss = self.criterion(squared_norm_preds.to(eps_pred.device), one.to(eps_pred.device)) + self.criterion(non_diag.to(eps_pred.device), torch.tensor(0.0, requires_grad=True).to(eps_pred.device))

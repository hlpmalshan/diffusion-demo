...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        self.lr = abs(lr)
        self.iter_num_step = iter_num_step
    ...
    def loss(self, x, epoch, epochs):
        ...
        squared_norm_preds = torch.mean(torch.sum(eps_pred**2, dim=len(eps_pred.shape)-1))/len(x.shape[1:])
        dim_ = torch.tensor(1.0, requires_grad=True)
    ...
    def train_step(self, x_batch, epoch, num_epochs=1000):
        self.optimizer.zero_grad()
        loss, simple_diff_loss, norm_loss = self.loss(x_batch, epoch, num_epochs)
        loss.backward()
        self.optimizer.step()
